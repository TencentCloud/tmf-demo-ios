...
- (void)manualAnimationWithCurrentOffsetY:(CGFloat)currentOffsetY
    ...
    for (NSInteger i = 0, l = _lines.count; i < l; i++) {
        ...
        _currentOffsetTime = ((-currentOffsetY + beginAnimationOffset) / distanceForCompleteAnimation) * AnimationDuration
                             * 0.4;  // timeOffset为0.6时loading刚好走完一轮，所以这里按总时间 * 0.4，从而保证loading停靠在顶部时，timeOffset刚好为0.6
        //        NSLog(@"_currentOffsetTime = %.2f, currentOffsetY = %.2f, distanceForStartRefresh = %.2f, distanceForCompleteAnimation = %.2f",
        //        (float)_currentOffsetTime, currentOffsetY, distanceForStartRefresh, distanceForCompleteAnimation);

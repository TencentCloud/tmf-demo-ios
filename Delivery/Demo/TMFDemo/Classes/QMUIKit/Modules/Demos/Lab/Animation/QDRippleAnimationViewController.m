...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleWillEnterForeground:)
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    _initPath =
        [UIBezierPath bezierPathWithOvalInRect:CGRectInset(CGRectMake(0, 0, RippleAnimationAvatarSize.width, RippleAnimationAvatarSize.height),
                                                   RippleAnimationLineWidth, RippleAnimationLineWidth)];
    _finalPath = [UIBezierPath bezierPathWithOvalInRect:CGRectInset(CGRectMake(-RippleAnimationExpandSizeValue, -RippleAnimationExpandSizeValue,
                                                                        RippleAnimationAvatarSize.width + RippleAnimationExpandSizeValue * 2,
                                                                        RippleAnimationAvatarSize.height + RippleAnimationExpandSizeValue * 2),
                                                            RippleAnimationLineWidth, RippleAnimationLineWidth)];
...
- (void)viewDidLayoutSubviews {
    ...
    _avatarWrapView1.frame = CGRectMake(CGFloatGetCenter(CGRectGetWidth(self.view.bounds), RippleAnimationAvatarSize.width),
        CGRectGetMaxY(_textLabel.frame) + 70, RippleAnimationAvatarSize.width, RippleAnimationAvatarSize.height);
    _avatarWrapView2.frame = CGRectMake(CGRectGetMinX(_avatarWrapView1.frame), CGRectGetMaxY(_avatarWrapView1.frame) + 100,
        RippleAnimationAvatarSize.width, RippleAnimationAvatarSize.height);
...
- (void)animationAvatarInView:(UIView *)view animated:(BOOL)animated {
    ...
    for (int i = 0; i < count; i++) {
        if ([view.layer.sublayers[i] isKindOfClass:[CAShapeLayer class]]) {
            [layers addObject:view.layer.sublayers[i]];

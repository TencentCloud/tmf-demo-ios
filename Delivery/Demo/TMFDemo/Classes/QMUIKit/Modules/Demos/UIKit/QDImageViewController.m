...
- (void)initDataSource {
    self.dataSourceWithDetailText = [[QMUIOrderedDictionary alloc]
        initWithKeysAndObjects:@"- qmui_averageColor", @"获取整张图片的平均颜色", @"- qmui_grayImage", @"将图片置灰",
        @"- qmui_imageWithAlpha:", @"调整图片的alpha值，返回一张新图片", @"- qmui_imageWithTintColor:", @"更改图片的颜色，只支持按路径来渲染图片",
        ...
        @"- qmui_imageWithClippedCornerRadius:", @"将图片按指定圆角裁剪出来，返回裁剪出来的区域", @"- qmui_imageResizedInLimitedSize:resizingMode:",
        @"将当前图片缩放到指定的大小，缩放策略可以指定不同的resizingMode，经过缩放后的图片倍数保持不变",
        ...
        @"+ qmui_imageWithAttributedString:", @"将给定的NSAttributedString渲染为图片（单行）", @"+ qmui_imageWithView:", @"生成给定View的截图",
        @"+ qmui_imageWithView:afterScreenUpdates:", @"在当前runloop更新后再生成给定View的截图", nil];
}
...
- (void)didSelectCellWithTitle:(NSString *)title {
    ...
    } else if ([title isEqualToString:@"+ qmui_imageWithShape:size:tintColor:"] ||
               [title isEqualToString:@"+ qmui_imageWithShape:size:lineWidth:tintColor:"]) {
...
- (CGFloat)generateExampleViewForAverageColor {
    ...
    originImageView.frame
        = CGRectMake(0, minY, contentWidth, flat(contentWidth * originImageView.image.size.height / originImageView.image.size.width));
    ...
    afterLabel.text = [NSString stringWithFormat:@"计算出的平均色：RGB(%d, %d, %d)", (int)(qmui_averageColor.qmui_red * 255),
                                (int)(qmui_averageColor.qmui_green * 255), (int)(qmui_averageColor.qmui_blue * 255)];
...
- (CGFloat)generateExampleViewForImageWithAlpha {
    ...
    UIImageView *afterImageView2 =
        [[UIImageView alloc] initWithFrame:CGRectMake(20, CGRectGetMinY(afterImageView.frame) + 20, imageViewSize.width, imageViewSize.height)];
...
- (CGFloat)generateExampleViewForImageWithBlendColor {
    ...
    UIImageView *originImageView =
        [[UIImageView alloc] initWithImage:[UIImageMake(@"image0") qmui_imageResizedInLimitedSize:CGSizeMake(contentWidth, contentWidth)]];
    originImageView.contentMode = UIViewContentModeScaleAspectFit;
    originImageView.frame = CGRectSetY(originImageView.frame, minY);
    [self.scrollView addSubview:originImageView];
    minY = CGRectGetMaxY(originImageView.frame) + 16;
...
- (CGFloat)generateExampleViewForImageWithImageAbove {
    ...
    UIImageView *originImageView = [[UIImageView alloc]
        initWithImage:[UIImageMake(@"icon_emotion") qmui_imageWithTintColor:[QDThemeManager sharedInstance].currentTheme.themeTintColor]];
    originImageView.contentMode = UIViewContentModeScaleAspectFit;
    originImageView.frame = CGRectSetY(originImageView.frame, minY);
    [self.scrollView addSubview:originImageView];
    minY = CGRectGetMaxY(originImageView.frame) + 16;
    ...
    UIImage *afterImage =
        [originImageView.image qmui_imageWithImageAbove:redDotImage
                                                atPoint:CGPointMake(originImageView.image.size.width - redDotImage.size.width - 1, 1)];
    UIImageView *afterImageView = [[UIImageView alloc] initWithImage:afterImage];
    afterImageView.contentMode = originImageView.contentMode;
    afterImageView.frame = CGRectSetY(afterImageView.frame, minY);
    [self.scrollView addSubview:afterImageView];
...
- (CGFloat)generateExampleViewForImageWithClippedCornerRadius {
    ...
    originImage = [originImage qmui_imageWithClippedRect:CGRectFlatMake(CGFloatGetCenter(originImage.size.width, 200),
                                                             CGFloatGetCenter(originImage.size.height, 200), 200, 200)];
...
- (CGFloat)generateExampleViewForImageWithClippedRect {
    ...
    UIImage *afterImage =
        [originImageView.image qmui_imageWithClippedRect:CGRectMake(originImageView.image.size.width / 4, originImageView.image.size.height / 4,
                                                             originImageView.image.size.width / 2, originImageView.image.size.height / 2)];
    UIImageView *afterImageView = [[UIImageView alloc] initWithImage:afterImage];
    afterImageView.contentMode = originImageView.contentMode;
    afterImageView.frame = CGRectSetY(afterImageView.frame, minY);
    [self.scrollView addSubview:afterImageView];
...
- (CGFloat)generateExampleViewForResizedImage {
    ...
    // 对原图进行缩放操作，以保证缩放后的图的 size 不超过 limitedSize 的大小，至于缩放策略则由 resizingMode 决定。resizingMode 默认是
    // QMUIImageResizingModeScaleAspectFit。 特别的，对于 ScaleAspectFit 类型，你可以对不关心大小的那一边传 CGFLOAT_MAX
    // 来表示“我不关心这一边缩放后的大小限制”，但对其他类型的 contentMode 则宽高都必须传一个确切的值。
    UIImage *afterImage = [originImageView.image qmui_imageResizedInLimitedSize:CGSizeMake(100, 50)
...
- (CGFloat)generateExampleViewForImageWithBorder {
    ...
    UIBezierPath *roundedBorderPath =
        [UIBezierPath bezierPathWithRoundedRect:CGRectInset(CGRectMakeWithSize(originImageView.image.size), lineWidth / 2.0, lineWidth / 2.0)
                                   cornerRadius:4];
...
- (CGFloat)generateExampleViewForImageWithBorderColorAndCornerRadiusAndPosition {
    ...
    UIImage *afterImage = [originImageView.image qmui_imageWithBorderColor:[QDCommonUI randomThemeColor] borderWidth:PixelOne
                                                            borderPosition:QMUIImageBorderPositionBottom];
...
- (CGFloat)generateExampleViewForImageWithColorAndCornerRadiusArray {
    ...
    UIImage *originImage = [UIImage qmui_imageWithColor:[QDCommonUI randomThemeColor] size:CGSizeMake(contentWidth / 2, 40)
                                      cornerRadiusArray:@[@0, @0, @10, @10]];
    UIImageView *originImageView = [[UIImageView alloc] initWithImage:originImage];
    originImageView.frame = CGRectSetY(originImageView.frame, minY);
    [self.scrollView addSubview:originImageView];
    minY = CGRectGetMaxY(originImageView.frame) + 16;
...
- (CGFloat)generateExampleViewForImageWithStrokeColorAndPath {
    ...
    UIBezierPath *path =
        [UIBezierPath bezierPathWithOvalInRect:CGRectMake(lineWidth / 2.0, lineWidth / 2.0, contentWidth / 2 - lineWidth, 40 - lineWidth)];
    path.lineWidth = lineWidth;
    UIImage *originImage = [UIImage qmui_imageWithStrokeColor:[QDCommonUI randomThemeColor] size:CGSizeMake(contentWidth / 2, 40) path:path
                                                      addClip:NO];
    UIImageView *originImageView = [[UIImageView alloc] initWithImage:originImage];
    originImageView.frame = CGRectSetY(originImageView.frame, minY);
    [self.scrollView addSubview:originImageView];
...
- (CGFloat)generateExampleViewForImageWithStrokeColorAndCornerRadius {
    ...
    UIImage *originImage = [UIImage qmui_imageWithStrokeColor:[QDCommonUI randomThemeColor] size:CGSizeMake(contentWidth / 2, 40) lineWidth:1
                                                 cornerRadius:10];
    UIImageView *originImageView = [[UIImageView alloc] initWithImage:originImage];
    originImageView.frame = CGRectSetY(originImageView.frame, minY);
    [self.scrollView addSubview:originImageView];
...
- (CGFloat)generateExampleViewForImageWithStrokeColorAndBorderPosition {
    ...
    UIImage *originImage =
        [UIImage qmui_imageWithStrokeColor:[QDCommonUI randomThemeColor] size:CGSizeMake(contentWidth / 2, 40) lineWidth:1
                            borderPosition:QMUIImageBorderPositionLeft | QMUIImageBorderPositionRight | QMUIImageBorderPositionBottom];
    UIImageView *originImageView = [[UIImageView alloc] initWithImage:originImage];
    originImageView.frame = CGRectSetY(originImageView.frame, minY);
    [self.scrollView addSubview:originImageView];
    minY = CGRectGetMaxY(originImageView.frame) + 16;

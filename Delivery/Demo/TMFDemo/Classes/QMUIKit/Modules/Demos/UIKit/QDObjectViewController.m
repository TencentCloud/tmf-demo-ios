...
- (double)matchingWeightForResult:(NSString *)className withSearchString:(NSString *)searchString {
    /**
     排序方式：
     1. 每个 className 都有一个基准的匹配权重，权重取值范围 [0-1]，这个权重简单地以字符串长度来计算，匹配到的 className 里长度越短的 className
     认为匹配度越高
     2. 基于步骤 1 得到的匹配权重进行分段，以搜索词开头的 className 权重最高，以下划线开头的 className
     权重最低（如果搜索词本来就以下划线开头则不计入此种情况），其他情况权重中等。
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSMutableAttributedString *attributedString =
        [[NSMutableAttributedString alloc] initWithString:className
                                               attributes:@{ NSFontAttributeName: CodeFontMake(14), NSForegroundColorAttributeName: UIColorGray1 }];
    [attributedString addAttribute:NSForegroundColorAttributeName value:[QDThemeManager sharedInstance].currentTheme.themeTintColor
                             range:matchingRange];
    ...
    if (searchString.length > 2) {
        ...
        [self.autocompletionClasses sortUsingComparator:^NSComparisonResult(NSString *obj1, NSString *obj2) {
            ...
            NSComparisonResult result
                = matchingWeight1 == matchingWeight2 ? NSOrderedSame : (matchingWeight1 > matchingWeight2 ? NSOrderedAscending : NSOrderedDescending);
            return result;
        }];
    }

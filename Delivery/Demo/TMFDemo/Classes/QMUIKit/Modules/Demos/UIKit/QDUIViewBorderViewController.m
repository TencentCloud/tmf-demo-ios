...
- (QMUIButton *)generateSelectableButtonWithTitle:(NSString *)title {
    ...
    UIImage *selectedImage = [TableViewCellCheckmarkImage qmui_imageResizedInLimitedSize:CGSizeMake(13, 13)
                                                                            resizingMode:QMUIImageResizingModeScaleAspectFit];
...
- (void)viewDidLayoutSubviews {
    ...
    if (!IS_IPAD && IS_LANDSCAPE) {
        ...
        self.targetView.qmui_top = CGFloatGetCenter(
            CGRectGetHeight(self.view.bounds) - UIEdgeInsetsGetVerticalValue(self.view.qmui_safeAreaInsets), CGRectGetHeight(self.targetView.frame));
    ...
    self.locationSegmentedControl.center
        = CGPointMake(CGRectGetWidth(self.scrollView.bounds) - marginRight - CGRectGetWidth(self.locationSegmentedControl.frame) / 2.0,
            self.locationTitleLabel.center.y);
    ...
    maxY += defaultLineHeight;
    [@[self.positionTopButton, self.positionLeftButton, self.positionBottomButton, self.positionRightButton]
        enumerateObjectsUsingBlock:^(QMUIButton *obj, NSUInteger idx, BOOL *_Nonnull stop) {
    ...
    self.colorSegmentedControl.center = CGPointMake(
        CGRectGetWidth(self.scrollView.bounds) - marginRight - CGRectGetWidth(self.colorSegmentedControl.frame) / 2.0, self.colorTitleLabel.center.y);
    maxY += defaultLineHeight;
    ...
    maxY += defaultLineHeight;
    [@[self.maskedCornersMinXMinYButton, self.maskedCornersMaxXMinYButton, self.maskedCornersMinXMaxYButton, self.maskedCornersMaxXMaxYButton]
        enumerateObjectsUsingBlock:^(QMUIButton *obj, NSUInteger idx, BOOL *_Nonnull stop) {
            obj.contentEdgeInsets = UIEdgeInsetsMake(0, marginLeft + 18, 0, marginRight);
            obj.qmui_top = maxY;
            obj.qmui_width = CGRectGetWidth(self.scrollView.bounds);
...
- (void)handleSelectableButtonEvent:(QMUIButton *)button {
    ...
    if (button == self.positionTopButton || button == self.positionLeftButton || button == self.positionBottomButton
        || button == self.positionRightButton) {
...
- (void)fireAllEvents {
    [@[
        self.locationSegmentedControl, self.widthTextField, self.colorSegmentedControl, self.dashPatternWidthTextField,
        self.dashPatternSpacingTextField, self.dashPhaseTextField
    ] enumerateObjectsUsingBlock:^(UIControl *obj, NSUInteger idx, BOOL *_Nonnull stop) {
        [obj sendActionsForControlEvents:UIControlEventValueChanged];
    }];
    [@[self.widthTextField, self.dashPatternWidthTextField, self.dashPatternSpacingTextField, self.dashPhaseTextField]
        enumerateObjectsUsingBlock:^(UIControl *obj, NSUInteger idx, BOOL *_Nonnull stop) {
...
- (void)handleGestureRecognizer:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer.state == UIGestureRecognizerStateBegan) {
        ...
        UIImage *snapshotImage = [UIImage qmui_imageWithSize:size opaque:NO scale:ScreenScale * scale actions:^(CGContextRef contextRef) {
            ...
            // 当你使用了 qmui_maskedCorners 并且只指定某几个角为圆角时，用以下这种方式绘制出来的图片会让直角也绘制为圆角，这是 iOS 11 及以上的系统
            // bug，暂不处理
    ...
    locationInView.x = MIN(MAX(locationInView.x, CGRectGetMinX(self.targetView.frame) - self.targetView.qmui_borderWidth),
        CGRectGetMaxX(self.targetView.frame) + self.targetView.qmui_borderWidth);
    locationInView.y = MIN(MAX(locationInView.y, CGRectGetMinY(self.targetView.frame) - self.targetView.qmui_borderWidth),
        CGRectGetMaxY(self.targetView.frame) + self.targetView.qmui_borderWidth);
    ...
    self.magnifyingImageView.center = CGPointMake(CGRectGetWidth(self.magnifyingView.bounds) / 2 + CGRectGetWidth(self.magnifyingImageView.frame) / 2
                                                      - offset * scale - locationInTarget.x * scale,
        CGRectGetHeight(self.magnifyingView.bounds) / 2 + CGRectGetHeight(self.magnifyingImageView.frame) / 2 - offset * scale
            - locationInTarget.y * scale);
...
- (void)keyboardWillChangeFrameWithUserInfo:(QMUIKeyboardUserInfo *)keyboardUserInfo {
    ...
    self.scrollView.scrollIndicatorInsets = UIEdgeInsetsSetBottom(
        self.scrollView.contentInset, self.scrollView.contentInset.bottom - (keyboardHeight > 0 ? self.scrollView.qmui_safeAreaInsets.bottom : 0));

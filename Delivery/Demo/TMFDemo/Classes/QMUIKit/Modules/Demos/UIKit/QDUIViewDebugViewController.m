...
- (void)initSubviews {
    ...
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]
        initWithString:@"通过 qmui_shouldShowDebugColor 让 UIView 以及其所有的 subviews 都加上一个背景色，方便查看其布局情况"
            attributes:@{
...
- (void)viewDidLayoutSubviews {
    ...
    UIEdgeInsets padding = UIEdgeInsetsMake(24 + self.qmui_navigationBarMaxYInViewCoordinator, 24 + self.view.qmui_safeAreaInsets.left,
        24 + self.view.qmui_safeAreaInsets.bottom, 24 + self.view.qmui_safeAreaInsets.right);
    self.descriptionLabel.frame
        = CGRectMake(padding.left, padding.top, CGRectGetWidth(self.view.bounds) - UIEdgeInsetsGetHorizontalValue(padding), QMUIViewSelfSizingHeight);
    ...
    self.parentView.frame = CGRectMake(
        padding.left, CGRectGetMaxY(self.descriptionLabel.frame) + 24, CGRectGetWidth(self.descriptionLabel.frame), self.subview2.qmui_bottom + 24);
}

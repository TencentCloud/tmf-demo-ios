...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat inset = 12;
    self.textCountLabel.frame = CGRectMake(
        inset, self.qmui_navigationBarMaxYInViewCoordinator + 20, CGRectGetWidth(self.view.bounds) - 2 * inset, QMUIViewSelfSizingHeight);
...
- (BOOL)canPopViewController {
    ...
    if (self.textView.text.length > 0) {
        ...
        QMUIAlertController *alertController = [QMUIAlertController alertControllerWithTitle:@"是否返回？" message:@"返回后输入框的数据将不会自动保存"
                                                                              preferredStyle:QMUIAlertControllerStyleAlert];
        QMUIAlertAction *backActioin = [QMUIAlertAction actionWithTitle:@"返回" style:QMUIAlertActionStyleCancel
                                                                handler:^(QMUIAlertController *aAlertController, QMUIAlertAction *action) {
                                                                    [self.navigationController popViewControllerAnimated:YES];
                                                                }];
        QMUIAlertAction *continueAction = [QMUIAlertAction actionWithTitle:@"继续编辑" style:QMUIAlertActionStyleDefault
                                                                   handler:^(QMUIAlertController *aAlertController, QMUIAlertAction *action) {
                                                                       [self.textView becomeFirstResponder];
                                                                   }];

...
- (void)initDataSource {
    self.dataSource = [[QMUIOrderedDictionary alloc]
        initWithKeysAndObjects:kSectionTitleForAlert,
        [[QMUIOrderedDictionary alloc] initWithKeysAndObjects:@"显示一个 alert 弹窗", @"", @"支持 alert 背景磨砂", @"建议用 QMUIVisualEffectView",
                                       @"支持自定义 alert 样式", @"支持以 UIAppearance 方式设置全局统一样式", @"支持自定义内容",
                                       @"可以将一个 UIView 作为 QMUIAlertController 的 contentView", nil],
        kSectionTitleForActionSheet,
        [[QMUIOrderedDictionary alloc] initWithKeysAndObjects:@"显示一个 actionSheet 菜单", @"", @"支持 actionSheet 背景磨砂",
                                       @"建议用 QMUIVisualEffectView", @"支持自定义 actionSheet 样式", @"支持以 UIAppearance 方式设置全局统一样式",
                                       nil],
        kSectionTitleForSystem,
        [[QMUIOrderedDictionary alloc] initWithKeysAndObjects:@"显示一个系统的 alert 弹窗", @"", @"显示一个系统的 actionSheet 菜单", @"", nil], nil];
}
...
- (void)didSelectCellWithTitle:(NSString *)title {
    ...
    if ([title isEqualToString:@"显示一个 alert 弹窗"]) {
        ...
        QMUIAlertController *alertController = [QMUIAlertController alertControllerWithTitle:@"确定删除？" message:@"删除后将无法恢复，请慎重考虑"
    ...
    if ([title isEqualToString:@"支持自定义 alert 样式"]) {
        ...
        [action2.button setImage:[[UIImageMake(@"icon_emotion") qmui_imageResizedInLimitedSize:CGSizeMake(18, 18)
        ...
        // 弹窗
        QMUIAlertController *alertController = [[QMUIAlertController alloc] initWithTitle:@"确定删除？" message:@"删除后将无法恢复，请慎重考虑"
                                                                           preferredStyle:QMUIAlertControllerStyleAlert];
    ...
    if ([title isEqualToString:@"支持自定义内容"]) {
        ...
        QMUIAlertController *alertController = [QMUIAlertController alertControllerWithTitle:@"正在加载" message:@"加载结束之前请勿取消"
                                                                              preferredStyle:QMUIAlertControllerStyleAlert];
        [alertController addAction:action1];
        [alertController addAction:action2];
    ...
    if ([title isEqualToString:@"显示一个 actionSheet 菜单"]) {
        QMUIAlertAction *action1 = [QMUIAlertAction actionWithTitle:@"取消" style:QMUIAlertActionStyleCancel
                                                            handler:^(QMUIAlertController *aAlertController, QMUIAlertAction *action) {
                                                            }];
        QMUIAlertAction *action2 = [QMUIAlertAction actionWithTitle:@"删除" style:QMUIAlertActionStyleDestructive
                                                            handler:^(QMUIAlertController *aAlertController, QMUIAlertAction *action) {
                                                            }];
        QMUIAlertAction *action3 = [QMUIAlertAction actionWithTitle:@"置灰按钮" style:QMUIAlertActionStyleDefault
                                                            handler:^(QMUIAlertController *aAlertController, QMUIAlertAction *action) {
                                                            }];
        action3.enabled = NO;
        QMUIAlertController *alertController = [QMUIAlertController alertControllerWithTitle:@"确定删除？" message:@"删除后将无法恢复，请慎重考虑"
    ...
    if ([title isEqualToString:@"支持自定义 actionSheet 样式"]) {
        QMUIAlertAction *action1 = [QMUIAlertAction actionWithTitle:@"取消" style:QMUIAlertActionStyleCancel handler:NULL];
        QMUIAlertAction *action2 = [QMUIAlertAction actionWithTitle:@"删除" style:QMUIAlertActionStyleDefault handler:NULL];
        [action2.button setImage:[[UIImageMake(@"icon_emotion") qmui_imageResizedInLimitedSize:CGSizeMake(22, 22)
                                                                                  resizingMode:QMUIImageResizingModeScaleToFill]
                                     qmui_imageWithTintColor:[QDThemeManager sharedInstance].currentTheme.themeTintColor]
                        forState:UIControlStateNormal];
        action2.button.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 8);
    ...
    if ([title isEqualToString:@"显示一个系统的 alert 弹窗"]) {
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"确定删除?" message:@"删除后将无法恢复，请慎重考虑"

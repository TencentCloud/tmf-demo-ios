...
- (void)initSubviews {
    ...
    [self.imagePositionButton1 setTitle:NSLocalizedString(@"QMUIButton_Image_Position_Button_Title_1", @"Text below image")
    ...
    [self.imagePositionButton2 setTitle:NSLocalizedString(@"QMUIButton_Image_Position_Button_Title_2", @"Text above image")
                               forState:UIControlStateNormal];
    ...
    self.imageButtonSeparatorLayer = [CAShapeLayer qmui_separatorDashLayerWithLineLength:3 lineSpacing:2 lineWidth:PixelOne
                                                                               lineColor:UIColorSeparator.CGColor
                                                                            isHorizontal:NO];
...
- (void)viewDidLayoutSubviews {
    ...
    // 普通按钮
    self.normalButton.frame
        = CGRectSetXY(self.normalButton.frame, CGFloatGetCenter(CGRectGetWidth(self.view.bounds), CGRectGetWidth(self.normalButton.frame)),
            contentMinY + CGFloatGetCenter(buttonSpacingHeight, CGRectGetHeight(self.normalButton.frame)));
    ...
    // 边框按钮
    self.borderedButton.frame = CGRectSetY(self.normalButton.frame,
        CGRectGetMaxY(self.separatorLayer.frame) + CGFloatGetCenter(buttonSpacingHeight, CGRectGetHeight(self.normalButton.frame)));

    // 图片+文字按钮
    self.imagePositionButton1.frame
        = CGRectFlatMake(0, contentMinY + buttonSpacingHeight * 2, CGRectGetWidth(self.view.bounds) / 2.0, buttonSpacingHeight);
    self.imagePositionButton2.frame = CGRectSetX(self.imagePositionButton1.frame, CGRectGetMaxX(self.imagePositionButton1.frame));

    self.imageButtonSeparatorLayer.frame = CGRectFlatMake(
        CGRectGetMaxX(self.imagePositionButton1.frame), CGRectGetMinY(self.imagePositionButton1.frame), PixelOne, buttonSpacingHeight);
}

...
- (void)layoutSubviews {
    ...
    if (self.scrollDirection == UICollectionViewScrollDirectionVertical) {
        self.prevLayer.frame
            = CGRectMake(0, CGRectGetHeight(self.contentView.bounds) * (1 - self.pagingThreshold), CGRectGetWidth(self.contentView.bounds), PixelOne);
        self.nextLayer.frame = CGRectSetY(self.prevLayer.frame, CGRectGetHeight(self.contentView.bounds) * self.pagingThreshold);
    } else {
        self.prevLayer.frame
            = CGRectMake(CGRectGetWidth(self.contentView.bounds) * (1 - self.pagingThreshold), 0, PixelOne, CGRectGetHeight(self.contentView.bounds));

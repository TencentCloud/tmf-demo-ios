...
- (void)didInitializeWithStyle:(UITableViewStyle)style {
    ...
    self.dataSource = [[QMUIOrderedDictionary alloc]
        initWithKeysAndObjects:@"张三 的想法",
        @"全局 UI 配置：只需要修改一份配置表就可以调整 App 的全局样式，包括颜色、导航栏、输入框、列表等。一处修改，全局生效。", @"李四 的想法",
        @"UIKit 拓展及版本兼容：拓展多个 UIKit 的组件，提供更加丰富的特性和功能，提高开发效率；解决不同 iOS 版本常见的兼容性问题。", @"王五 的想法",
        ...
        @"拓展及版本兼容：拓展多个 UIKit 的组件，提供更加丰富的特性和功能，提高开发效率；解决不同 iOS "
        @"版本常见的兼容性问题。\n高效的工具方法及宏：提供高效的工具方法，包括设备信息、动态字体、键盘管理、状态栏管理等，可以解决各种常见场景并大幅"
        @"度提升开发效率。",
...
- (void)initTableView {
    [super initTableView];
    // 为了展示效果，这里主动把 estimatedRowHeight 关闭，以与 QMUICellHeightKeyCache 区分开，如果你的 tableView 使用了 estimatedRowHeight，请使用
    // QMUICellHeightKeyCache，用法参考 QDCellHeightKeyCacheViewController
    self.tableView.estimatedRowHeight = 0;
}

- (void)setupNavigationItems {
    [super setupNavigationItems];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Reload" style:UIBarButtonItemStyleDone target:self
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return [tableView qmui_heightForCellWithIdentifier:kCellIdentifier cacheByKey:cachedKey configuration:^(QDDynamicHeightTableViewCell *cell) {
        [cell updateCellAppearanceWithIndexPath:indexPath];
        [cell renderWithNameText:[NSString stringWithFormat:@"%@ - %@", @(indexPath.row), keyName]
                     contentText:[self.dataSource objectForKey:keyName]];
    }];
}

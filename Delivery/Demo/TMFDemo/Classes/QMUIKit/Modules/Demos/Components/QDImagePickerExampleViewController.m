...
- (void)initDataSource {
    ...
    self.dataSource = [[QMUIOrderedDictionary alloc]
        initWithKeysAndObjects:@"选图控件使用示例",
        [[QMUIOrderedDictionary alloc] initWithKeysAndObjects:@"默认", @"选图控件包含相册列表，选图，预览大图三个界面", @"自定义",
                                       @"修改选图界面列数，预览大图界面 TopBar 背景色", nil],
        @"通过重载进行添加 subview 等较大型的改动",
        [[QMUIOrderedDictionary alloc] initWithKeysAndObjects:@"选择多张图片", @"模拟聊天发图，预览大图界面增加底部工具栏", @"选择单张图片",
                                       @"模拟设置头像，预览大图界面右上角增加按钮", nil],
        nil];
}
...
- (void)imagePickerViewController:(QMUIImagePickerViewController *)imagePickerViewController
    ...
    [QMUIImagePickerHelper updateLastestAlbumWithAssetsGroup:imagePickerViewController.assetsGroup ablumContentType:kAlbumContentType
                                                userIdentify:nil];

    [self sendImageWithImagesAssetArray:imagesAssetArray];
}

- (QMUIImagePickerPreviewViewController *)imagePickerPreviewViewControllerForImagePickerViewController:
...
- (void)imagePickerPreviewViewController:(QMUIImagePickerPreviewViewController *)imagePickerPreviewViewController
...
- (void)updateImageCountLabelForPreviewView:(QMUIImagePickerPreviewViewController *)imagePickerPreviewViewController {
    if (imagePickerPreviewViewController.view.tag == MultipleImagePickingTag) {
        QDMultipleImagePickerPreviewViewController *customImagePickerPreviewViewController
            = (QDMultipleImagePickerPreviewViewController *)imagePickerPreviewViewController;
...
- (void)imagePickerPreviewViewController:(QDMultipleImagePickerPreviewViewController *)imagePickerPreviewViewController
           sendImageWithImagesAssetArray:(NSMutableArray<QMUIAsset *> *)imagesAssetArray {
    // 储存最近选择了图片的相册，方便下次直接进入该相册
    [QMUIImagePickerHelper updateLastestAlbumWithAssetsGroup:imagePickerPreviewViewController.assetsGroup ablumContentType:kAlbumContentType
                                                userIdentify:nil];
...
- (void)imagePickerPreviewViewController:(QDSingleImagePickerPreviewViewController *)imagePickerPreviewViewController
    ...
    [imageAsset requestImageData:^(NSData *imageData, NSDictionary<NSString *, id> *info, BOOL isGif, BOOL isHEIC) {
        ...
        if (isGif) {
            ...
            if (isHEIC) {
                // iOS 11 中新增 HEIF/HEVC 格式的资源，直接发送新格式的照片到不支持新格式的设备，照片可能会无法识别，可以先转换为通用的 JPEG
                // 格式再进行使用。 详细请浏览：https://github.com/Tencent/QMUI_iOS/issues/224
...
- (void)sendImageWithImagesAssetArrayIfDownloadStatusSucceed:(NSMutableArray<QMUIAsset *> *)imagesAssetArray {
    if ([QMUIImagePickerHelper imageAssetsDownloaded:imagesAssetArray]) {
        ...
        // 使用 delay 模拟网络请求时长
        [self performSelector:@selector(showTipLabelWithText:) withObject:[NSString stringWithFormat:@"成功发送%@个资源", @([imagesAssetArray count])]
                   afterDelay:1.5];
    }
}

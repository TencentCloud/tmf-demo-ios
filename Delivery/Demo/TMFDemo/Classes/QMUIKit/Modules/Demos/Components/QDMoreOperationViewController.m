...
- (void)initDataSource {
    ...
    self.dataSourceWithDetailText = [[QMUIOrderedDictionary alloc]
        initWithKeysAndObjects:@"支持 item 多行显示", @"每个 item 可通过 selected 来切换不同状态", @"支持动态修改 item 位置",
        @"点击第二行 item 来修改第一行 item", @"支持修改皮肤样式（例如夜间模式）", @"通过 appearance 设置全局样式", nil];
}
...
- (void)didSelectCellWithTitle:(NSString *)title {
    ...
    if ([title isEqualToString:@"支持 item 多行显示"]) {
        ...
        moreOperationController.items = @[
            // 第一行
            @[
                [QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_shareFriend") title:@"分享给微信好友" handler:^(
                    QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                    [moreOperationController hideToBottom];  // 如果嫌每次都在 handler 里写 hideToBottom 烦，也可以直接把这句写到
                                                             // moreOperationController:didSelectItemView: 里，它可与 handler 共存
                }],
                [QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_shareMoment") title:@"分享到朋友圈" handler:^(
                    QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                    [moreOperationController hideToBottom];
                }],
                [QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_shareWeibo") title:@"分享到微博" handler:^(
                    QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                    [moreOperationController hideToBottom];
                }],
                [QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_shareQzone") title:@"分享到QQ空间" handler:^(
                    QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                    [moreOperationController hideToBottom];
                }],
                [QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_shareChat") title:@"分享到私信" handler:^(
                    QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                    [moreOperationController hideToBottom];
                }]
            ],
            ...
            @[
                [QMUIMoreOperationItemView
                    itemViewWithImage:UIImageMake(@"icon_moreOperation_collect")
                        selectedImage:UIImageMake(@"icon_moreOperation_notCollect")
                                title:@"收藏"
                        selectedTitle:@"取消收藏"
                              handler:^(QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                                  itemView.selected = !itemView.selected;  // 通过 selected 切换 itemView 的状态
                              }],
                [QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_report") title:@"反馈" handler:^(
                    QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                    [moreOperationController hideToBottom];
                }],
                [QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_openInSafari") title:@"在Safari中打开" handler:^(
                    QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                    [moreOperationController hideToBottom];
                }]
            ],
        ...
        [moreOperationController
            addItemView:[QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_add") title:@"添加" handler:^(
                            QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                // 动态添加 item
                ...
                [moreOperationController addItemView:[QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_shareQzone")
                                                                                            title:@"分享到QQ空间"
                                                                                          handler:NULL]
                                           inSection:sectionToAdd];
            }]
              inSection:1];

        // 再给第二行插入一个 item
        [moreOperationController
            addItemView:[QMUIMoreOperationItemView itemViewWithImage:UIImageMake(@"icon_moreOperation_remove") title:@"删除" handler:^(
                            QMUIMoreOperationController *moreOperationController, QMUIMoreOperationItemView *itemView) {
                // 动态减少 item
                ...
                if (moreOperationController.items.count > 1) {
                    [moreOperationController
                        removeItemViewAtIndexPath:[NSIndexPath indexPathForItem:moreOperationController.items[sectionToRemove].count - 1
                                                                      inSection:sectionToRemove]];
                }
            }]
              inSection:1];

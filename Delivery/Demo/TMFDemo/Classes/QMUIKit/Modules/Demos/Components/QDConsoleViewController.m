...
- (void)initSubviews {
    ...
    NSMutableAttributedString *tips = [[NSMutableAttributedString alloc]
        initWithString:@"[QMUIConsole log:xxx] 可以直接在屏幕上显示日志，通常用于一些重要信息但又不适合用 NSAssert "
                       @"提示的场景。可通过长按小圆钮关闭控制台。\n支持搜索或按 Level、Name 过滤日志，可以通过配置表 ShouldPrintQMUIWarnLogToConsole "
                       @"让 QMUILogWarn() 的内容也自动显示到 QMUIConsole 里（默认仅在 DEBUG 下打开）。"
            attributes:@{
...
- (void)viewDidLayoutSubviews {
    ...
    UIEdgeInsets paddings = UIEdgeInsetsMake(self.qmui_navigationBarMaxYInViewCoordinator + 24, 24 + self.view.qmui_safeAreaInsets.left,
        24 + self.view.qmui_safeAreaInsets.bottom, 24 + self.view.qmui_safeAreaInsets.right);
    CGFloat buttonSpacing = 16;
    self.printLogButton.frame = CGRectMake(paddings.left, paddings.top, CGRectGetWidth(self.view.bounds) - UIEdgeInsetsGetHorizontalValue(paddings),
        CGRectGetHeight(self.printLogButton.frame));
    ...
    self.tipsLabel.frame = CGRectMake(paddings.left, CGRectGetMaxY(self.printMultipleLogButton.frame) + 16,
        CGRectGetWidth(self.printMultipleLogButton.frame), QMUIViewSelfSizingHeight);
}
...
- (void)handlePrintMulitipleEvent:(QMUIButton *)button {
    ...
    [QMUIConsole logWithLevel:@"Warn" name:@"QMUITableView"
                    logString:[[NSAttributedString alloc] initWithString:@"QMUITableView's warn log" attributes:({
                        NSMutableDictionary<NSAttributedStringKey, id> *attributes = [QMUIConsole appearance].textAttributes.mutableCopy;
                        attributes[NSForegroundColorAttributeName] = [QDThemeManager sharedInstance].currentTheme.themeTintColor;
                        attributes;
                    })]];

...
- (void)viewDidLayoutSubviews {
    ...
    _takeVideoButton.frame
        = CGRectSetXY(_takeVideoButton.frame, CGFloatGetCenter(CGRectGetWidth(self.view.bounds), CGRectGetWidth(_takeVideoButton.frame)),
            CGFloatGetCenter(CGRectGetHeight(self.view.bounds), CGRectGetHeight(_takeVideoButton.frame)));
}

- (void)saveVideoToAlbumWithMediaInfo:(NSDictionary *)info {
    if (!self.actionSheet) {
        self.actionSheet = [QMUIAlertController alertControllerWithTitle:@"保存到指定相册" message:nil
                                 ...
                                 QMUIAlertAction *action =
                                     [QMUIAlertAction actionWithTitle:[resultAssetsGroup name] style:QMUIAlertActionStyleDefault handler:^(
                                         ...
                                         if (UIVideoAtPathIsCompatibleWithSavedPhotosAlbum(weakSelf.videoPath)) {
                                             QMUISaveVideoAtPathToSavedPhotosAlbumWithAlbumAssetsGroup(
                                                 weakSelf.videoPath, resultAssetsGroup, ^(QMUIAsset *asset, NSError *error) {
                                                     [QMUITips showSucceed:[NSString stringWithFormat:@"已保存到相册-%@", [resultAssetsGroup name]]
                                                                    inView:weakSelf.navigationController.view
                                                            hideAfterDelay:2];
                                                 });
                                         } else {

...
- (void)setupNavigationItems {
    ...
    self.navigationItem.rightBarButtonItems = @[
        [UIBarButtonItem qmui_itemWithTitle:self.collectionViewLayout.debug ? @"普通模式" : @"调试模式" target:self
                                     action:@selector(handleDebugItemEvent)],
        [UIBarButtonItem qmui_itemWithTitle:self.collectionViewLayout.scrollDirection == UICollectionViewScrollDirectionVertical ? @"水平" : @"垂直"
                                     target:self
                                     action:@selector(handleDirectionItemEvent)]
    ];
}
...
- (void)handleDirectionItemEvent {
    self.collectionViewLayout.scrollDirection = self.collectionViewLayout.scrollDirection == UICollectionViewScrollDirectionVertical
...
- (UIEdgeInsets)sectionInset {
    if (self.collectionViewLayout.debug) {
        ...
        CGFloat horizontalInset
            = (CGRectGetWidth(self.collectionView.bounds) - UIEdgeInsetsGetHorizontalValue(self.collectionView.qmui_contentInset) - itemSize.width)
              / 2;
        CGFloat verticalInset
            = (CGRectGetHeight(self.collectionView.bounds) - UIEdgeInsetsGetVerticalValue(self.collectionView.qmui_contentInset) - itemSize.height)
              / 2;
        return UIEdgeInsetsMake(verticalInset, horizontalInset, verticalInset,
            CGRectGetWidth(self.collectionView.bounds) - horizontalInset - itemSize.width
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    QDCollectionViewDemoCell *cell = (QDCollectionViewDemoCell *)[self.collectionView dequeueReusableCellWithReuseIdentifier:@"cell"
...
- (CGSize)collectionView:(UICollectionView *)collectionView
    ...
    CGSize size = CGSizeMake(CGRectGetWidth(collectionView.bounds) - UIEdgeInsetsGetHorizontalValue(self.collectionViewLayout.sectionInset)
                                 - UIEdgeInsetsGetHorizontalValue(self.collectionView.qmui_contentInset),
        CGRectGetHeight(collectionView.bounds) - UIEdgeInsetsGetVerticalValue(self.collectionViewLayout.sectionInset)

...
- (void)initSubviews {
    ...
    self.descriptionLabel.attributedText =
        [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@ 对所有 NSObject 有效，这里只拿 UITextField 展示。",
                                                             NSStringFromClass([QMUIMultipleDelegates class])]
                                        attributes:@{
                                            NSFontAttributeName: UIFontMake(12),
                                            NSForegroundColorAttributeName: UIColorGray6,
...
- (void)viewDidLayoutSubviews {
    ...
    UIEdgeInsets paddings = UIEdgeInsetsMake(24 + self.qmui_navigationBarMaxYInViewCoordinator, 24 + self.view.qmui_safeAreaInsets.left,
        24 + self.view.qmui_safeAreaInsets.bottom, 24 + self.view.qmui_safeAreaInsets.right);

    self.descriptionLabel.frame = CGRectMake(
        paddings.left, paddings.top, CGRectGetWidth(self.view.bounds) - UIEdgeInsetsGetHorizontalValue(paddings), QMUIViewSelfSizingHeight);

    self.textField.frame
        = CGRectMake(paddings.left, CGRectGetMaxY(self.descriptionLabel.frame) + 12, CGRectGetWidth(self.descriptionLabel.frame), 44);

    CGFloat delegatorMarginTop = 24;
    CGFloat delegatorHorizontalSpacing = 24;
    [self.delegator1.titleLabel sizeToFit];
    self.delegator1.titleLabel.frame = CGRectSetXY(
        self.delegator1.titleLabel.frame, CGRectGetMinX(self.textField.frame), CGRectGetMaxY(self.textField.frame) + delegatorMarginTop);
    self.delegator1.printLabel.frame
        = CGRectMake(CGRectGetMinX(self.delegator1.titleLabel.frame), CGRectGetMaxY(self.delegator1.titleLabel.frame) + 12,
            (CGRectGetWidth(self.textField.frame) - delegatorHorizontalSpacing) / 2, QMUIViewSelfSizingHeight);

    self.delegator2.printLabel.frame
        = CGRectSetX(self.delegator1.printLabel.frame, CGRectGetMaxX(self.delegator1.printLabel.frame) + delegatorHorizontalSpacing);
    CGFloat printLabelHeight =
        [self.delegator2.printLabel sizeThatFits:CGSizeMake(CGRectGetWidth(self.delegator2.printLabel.frame), CGFLOAT_MAX)].height;
    ...
    self.delegator2.titleLabel.frame = CGRectSetXY(
        self.delegator2.titleLabel.frame, CGRectGetMinX(self.delegator2.printLabel.frame), CGRectGetMinY(self.delegator1.titleLabel.frame));
}
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        self.printLabel.qmui_textAttributes =
            @{ NSFontAttributeName: UIFontMake(12), NSParagraphStyleAttributeName: [NSMutableParagraphStyle qmui_paragraphStyleWithLineHeight:20] };

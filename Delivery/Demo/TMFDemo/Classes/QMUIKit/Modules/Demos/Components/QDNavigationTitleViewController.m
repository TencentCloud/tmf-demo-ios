...
- (void)initPopupContainerViewIfNeeded {
    if (!self.popupMenuView) {
        ...
        self.popupMenuView.items = @[
            [QMUIPopupMenuButtonItem itemWithImage:UIImageMake(@"icon_emotion") title:@"分类 1" handler:nil],
...
- (void)initDataSource {
    self.dataSource = @[
        @"显示左边的 loading", @"显示右边的 accessoryView", @"显示副标题", @"切换为上下两行显示", @"水平方向的对齐方式",
        @"模拟标题的 loading 状态切换", @"标题搭配浮层使用的示例", @"显示 Debug 背景色"
    ];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.row) {
        ...
        case 1:
            ...
            self.titleView.accessoryType = self.titleView.accessoryType == QMUINavigationTitleViewAccessoryTypeNone
                                               ? QMUINavigationTitleViewAccessoryTypeDisclosureIndicator
                                               : QMUINavigationTitleViewAccessoryTypeNone;
            break;
        case 2:
            // 切换副标题的显示/隐藏
        ...
        case 3:
            // 切换主副标题的水平/垂直布局
            self.titleView.style = self.titleView.style == QMUINavigationTitleViewStyleDefault ? QMUINavigationTitleViewStyleSubTitleVertical
                                                                                               : QMUINavigationTitleViewStyleDefault;
            self.titleView.subtitle = self.titleView.style == QMUINavigationTitleViewStyleSubTitleVertical ? @"(副标题)" : self.titleView.subtitle;
            break;
        case 4:
            // 水平对齐方式
            {
                QMUIAlertController *alertController = [QMUIAlertController alertControllerWithTitle:@"水平对齐方式" message:nil
                                                                                      preferredStyle:QMUIAlertControllerStyleActionSheet];
                [alertController addAction:[QMUIAlertAction actionWithTitle:@"左对齐" style:QMUIAlertActionStyleDefault
                                                                    handler:^(QMUIAlertController *aAlertController, QMUIAlertAction *action) {
                                                                        self.titleView.contentHorizontalAlignment
                                                                            = UIControlContentHorizontalAlignmentLeft;
                                                                        self.horizontalAlignment = self.titleView.contentHorizontalAlignment;
                                                                        [self.tableView reloadData];
                                                                    }]];
                [alertController addAction:[QMUIAlertAction actionWithTitle:@"居中对齐" style:QMUIAlertActionStyleDefault
                                                                    handler:^(QMUIAlertController *aAlertController, QMUIAlertAction *action) {
                                                                        self.titleView.contentHorizontalAlignment
                ...
                [alertController addAction:[QMUIAlertAction actionWithTitle:@"右对齐" style:QMUIAlertActionStyleDefault
                                                                    handler:^(QMUIAlertController *aAlertController, QMUIAlertAction *action) {
                                                                        self.titleView.contentHorizontalAlignment
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.row) {
        case 0:
            ...
            break;
        case 1:
            cell.textLabel.text = self.titleView.accessoryType == QMUINavigationTitleViewAccessoryTypeNone ? @"显示右边的 accessoryView"
                                                                                                           : @"去掉右边的 accessoryView";
            break;
        case 2:
        ...
        case 3:
            ...
            break;
        case 4:
            cell.detailTextLabel.text = (self.horizontalAlignment == UIControlContentHorizontalAlignmentLeft
                                             ? @"左对齐"
                                             : (self.horizontalAlignment == UIControlContentHorizontalAlignmentRight ? @"右对齐" : @"居中对齐"));
            break;
    }
    return cell;
}

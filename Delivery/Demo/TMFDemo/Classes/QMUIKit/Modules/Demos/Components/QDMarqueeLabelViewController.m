...
- (void)initSubviews {
    ...
    self.noFadeAndQuickLabel =
        [self generateLabelWithText:@"通过 shouldFadeAtEdge = NO 可隐藏文字滚动时边缘的渐隐遮罩，通过 speed 属性可以调节滚动的速度"];
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat labelSpacing = (CGRectGetMinY(self.separatorLayer.frame) - paddings.top - paddings.bottom
                               - CGRectGetHeight(self.view.subviews.firstObject.frame) * self.view.subviews.count)
    ...
    self.collectionView.frame = CGRectMake(0, CGRectGetMaxY(self.separatorLayer.frame), CGRectGetWidth(self.view.bounds),
        CGRectGetHeight(self.view.bounds) - CGRectGetMaxY(self.separatorLayer.frame));
    self.collectionViewLayout.itemSize
        = CGSizeMake(CGRectGetWidth(self.collectionView.bounds) - UIEdgeInsetsGetHorizontalValue(self.collectionViewLayout.sectionInset),
            CGRectGetHeight(self.collectionView.bounds) - UIEdgeInsetsGetVerticalValue(self.collectionViewLayout.sectionInset));
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    QDMarqueeCollectionViewCell *cell = (QDMarqueeCollectionViewCell *)[collectionView dequeueReusableCellWithReuseIdentifier:@"cell"
...
- (void)layoutSubviews {
    ...
    self.label.frame = CGRectMake(24, CGFloatGetCenter(CGRectGetHeight(self.contentView.bounds), CGRectGetHeight(self.label.frame)),
        CGRectGetWidth(self.contentView.bounds) - 24 * 2, CGRectGetHeight(self.label.frame));
}

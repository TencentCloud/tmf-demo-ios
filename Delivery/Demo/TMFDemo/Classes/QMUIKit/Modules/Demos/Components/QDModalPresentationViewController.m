...
- (void)initDataSource {
    ...
    self.dataSource = [[QMUIOrderedDictionary alloc]
        initWithKeysAndObjects:kSectionTitleForUsing,
        [[QMUIOrderedDictionary alloc] initWithKeysAndObjects:@"showWithAnimated", @"以 UIWindow 的形式盖在当前界面上", @"presentViewController",
                                       @"以 presentViewController: 的方式显示", @"showInView",
                                       @"以 addSubview: 的方式直接将浮层添加到要显示的 UIView 上", nil],
        kSectionTitleForStyling,
        [[QMUIOrderedDictionary alloc] initWithKeysAndObjects:@"contentView", @"直接显示一个UIView浮层", @"contentViewController",
                                       @"显示一个UIViewController", @"animationStyle", @"默认提供3种动画，可重复点击，依次展示", @"dimmingView",
                                       @"自带背景遮罩，也可自行制定一个遮罩的UIView", @"layoutBlock",
                                       @"利用layoutBlock、showingAnimation、hidingAnimation制作自定义的显示动画", @"keyboard",
                                       @"控件自带对keyboard的管理，并且能保证浮层和键盘同时升起，不会有跳动", nil],
        nil];
}
...
- (void)handleShowContentView {
    ...
    NSMutableAttributedString *attributedString =
        [[NSMutableAttributedString alloc] initWithString:@"默认的布局是上下左右居中，可通过contentViewMargins、maximumContentViewWidth属性来调整宽高"
                                                          @"、上下左右的偏移。\n你现在可以试试旋转一下设备试试看。"
                                               attributes:@{
    ...
    label.frame = CGRectMake(contentViewPadding.left, contentViewPadding.top,
        CGRectGetWidth(contentView.bounds) - UIEdgeInsetsGetHorizontalValue(contentViewPadding), QMUIViewSelfSizingHeight);
...
- (void)handleAnimationStyle {
    ...
    paragraphStyle.paragraphSpacing = 16;
    NSMutableAttributedString *attributedString =
        [[NSMutableAttributedString alloc] initWithString:@"modalViewController提供的显示/"
                                                          @"隐藏动画总共有3种，可通过animationStyle属性来设置，默认为QMUIModalPresentationAnimationSt"
...
- (void)handleCustomDimmingView {
    ...
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]
        initWithString:@"QMUIModalPresentationViewController允许自定义背景遮罩的dimmingView，例如这里的背景遮罩是拿当前界面进行截图磨砂后显示出来的。"
            attributes:@{
                NSFontAttributeName: UIFontMake(16),
                NSForegroundColorAttributeName: UIColorBlack,
                NSParagraphStyleAttributeName: paragraphStyle
...
- (void)handleLayoutBlockAndAnimation {
    ...
    paragraphStyle.paragraphSpacing = 16;
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]
        initWithString:@"利用layoutBlock可以自定义浮层的布局，注意此时contentViewMargins、maximumContentViewWidth属性均无效，如果需要实现外间距、最大"
                       @"宽高的保护，请自行计算。\n另外搭配showingAnimation、hidingAnimation也可制作自己的显示/"
                       @"隐藏动画，例如这个例子里实现了一个从底部升起的面板，升起后停靠在容器底端，你可以试着旋转设备，会发现依然能正确布局。"
            attributes:@{
                NSFontAttributeName: UIFontMake(16),
                NSForegroundColorAttributeName: UIColorBlack,
                NSParagraphStyleAttributeName: paragraphStyle
    ...
    modalViewController.layoutBlock = ^(CGRect containerBounds, CGFloat keyboardHeight, CGRect contentViewDefaultFrame) {
        contentView.frame = CGRectSetXY(contentView.frame, CGFloatGetCenter(CGRectGetWidth(containerBounds), CGRectGetWidth(contentView.frame)),
            CGRectGetHeight(containerBounds) - 20 - CGRectGetHeight(contentView.frame));
    };
    modalViewController.showingAnimation
        = ^(UIView *dimmingView, CGRect containerBounds, CGFloat keyboardHeight, CGRect contentViewFrame, void (^completion)(BOOL finished)) {
...
- (void)handleKeyboard {
    ...
    QMUITextField *textField =
        [[QMUITextField alloc] initWithFrame:CGRectMake(contentViewPadding.left, contentViewPadding.top, contentLimitWidth, 36)];
    ...
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]
        initWithString:
            @"如果你的浮层里有输入框，建议在把输入框添加到界面上后立即调用becomeFirstResponder（如果你用contentViewController，则在viewWillAppear:"
            @"时调用becomeFirstResponder），以保证键盘跟随浮层一起显示。\n而在浮层消失时，modalViewController会自动降下键盘，所以你的浮层里并不需要处"
            @"理。"
            attributes:@{
                NSFontAttributeName: UIFontMake(12),
                NSForegroundColorAttributeName: UIColorGrayDarken,
                NSParagraphStyleAttributeName: paragraphStyle
            }];

    NSDictionary *codeAttributes = @{
        NSFontAttributeName: CodeFontMake(12),
        NSForegroundColorAttributeName: [[QDThemeManager sharedInstance].currentTheme.themeCodeColor colorWithAlphaComponent:.8]
    };
    [attributedString.string enumerateCodeStringUsingBlock:^(NSString *codeString, NSRange codeRange) {
        [attributedString addAttributes:codeAttributes range:codeRange];
    }];
...
- (void)handleWindowShowing {
    ...
    paragraphStyle.paragraphSpacing = 16;
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]
        initWithString:@"QMUIModalPresentationViewController支持 3 种使用方式，当前使用第 1 种，注意状态栏被遮罩盖住了"
            attributes:@{
                NSFontAttributeName: UIFontMake(16),
                NSForegroundColorAttributeName: UIColorBlack,
                NSParagraphStyleAttributeName: paragraphStyle
...
- (void)handlePresentShowing {
    ...
    paragraphStyle.paragraphSpacing = 16;
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]
        initWithString:@"QMUIModalPresentationViewController支持 3 种使用方式，当前使用第 2 种，注意遮罩无法盖住屏幕顶部的状态栏。"
            attributes:@{
                NSFontAttributeName: UIFontMake(16),
                NSForegroundColorAttributeName: UIColorBlack,
                NSParagraphStyleAttributeName: paragraphStyle
...
- (void)handleShowInView {
    ...
    CGRect modalRect = CGRectMake(40, self.qmui_navigationBarMaxYInViewCoordinator + 40, CGRectGetWidth(self.view.bounds) - 40 * 2,
        CGRectGetHeight(self.view.bounds) - self.qmui_navigationBarMaxYInViewCoordinator - 40 * 2);
    ...
    paragraphStyle.paragraphSpacing = 16;
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]
        initWithString:@"QMUIModalPresentationViewController支持 3 种使用方式，当前使用第 3 种，注意可以透过遮罩外的空白地方点击到背后的 cell"
            attributes:@{
                NSFontAttributeName: UIFontMake(16),
                NSForegroundColorAttributeName: UIColorBlack,
                NSParagraphStyleAttributeName: paragraphStyle
...
- (void)viewDidLoad {
    ...
    paragraphStyle.paragraphSpacing = 16;
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc]
        initWithString:@"如果你的浮层是以UIViewController的形式存在的，那么就可以通过modalViewController."
                       @"contentViewController属性来显示出来。\n利用UIViewController的特点，你可以方便地管理复杂的UI状态，并且响应设备在不同状态下的"
...
- (void)viewDidLayoutSubviews {
    ...
    CGSize contentSize = CGSizeMake(CGRectGetWidth(self.view.bounds) - UIEdgeInsetsGetHorizontalValue(padding),
        CGRectGetHeight(self.view.bounds) - UIEdgeInsetsGetVerticalValue(padding));
    ...
    if (IS_LANDSCAPE) {
        ...
        self.textLabel.frame = CGRectMake(CGRectGetMaxX(self.imageView.frame) + textLabelMarginLeft, padding.top - 6,
            contentSize.width - CGRectGetWidth(self.imageView.frame) - textLabelMarginLeft, QMUIViewSelfSizingHeight);
    } else {
        // 竖屏下图文垂直布局
        ...
        self.textLabel.frame
            = CGRectMake(padding.left, CGRectGetMaxY(self.imageView.frame) + textLabelMarginTop, contentSize.width, QMUIViewSelfSizingHeight);
    }
...
- (CGSize)preferredContentSizeInModalPresentationViewController:(QMUIModalPresentationViewController *)controller limitSize:(CGSize)limitSize {
    ...
    return CGSizeMake(CGRectGetWidth(controller.view.bounds) - UIEdgeInsetsGetHorizontalValue(controller.view.qmui_safeAreaInsets)
                          - UIEdgeInsetsGetHorizontalValue(controller.contentViewMargins),
        CGRectGetHeight(controller.view.bounds) - UIEdgeInsetsGetVerticalValue(controller.view.qmui_safeAreaInsets)
            - UIEdgeInsetsGetVerticalValue(controller.contentViewMargins));
}

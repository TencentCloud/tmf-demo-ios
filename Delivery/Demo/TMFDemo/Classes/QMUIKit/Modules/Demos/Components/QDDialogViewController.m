...
- (instancetype)initWithStyle:(UITableViewStyle)style {
    if (self = [super initWithStyle:style]) {
        self.dataSource = [[QMUIOrderedDictionary alloc]
            initWithKeysAndObjects:kSectionTitleForNormal,
            [[QMUIOrderedDictionary alloc]
                initWithKeysAndObjects:@"普通弹窗", @"", @"支持自定义样式", @"可通过 appearance 方式来统一修改全局样式", nil],
            kSectionTitleForSelection,
            [[QMUIOrderedDictionary alloc] initWithKeysAndObjects:@"列表弹窗", @"支持显示一个列表", @"支持单选", @"最多只能勾选一个 item，不可不选",
                                           @"支持多选", @"可同时勾选多个 item，可全部取消勾选", nil],
            kSectionTitleForTextField,
            [[QMUIOrderedDictionary alloc] initWithKeysAndObjects:@"输入框弹窗", @"", @"支持通过键盘 Return 按键触发弹窗提交按钮事件",
                                           @"默认开启，当需要自己管理输入框 shouldReturn 事件时请将其关闭", @"支持自动控制提交按钮的 enable 状态",
                                           @"默认开启，只要文字不为空则允许点击", @"支持自定义提交按钮的 enable 状态", @"通过 block 来控制状态", nil],
            nil];
    }
    return self;
}
...
- (void)showAppearanceDialogViewController {
    ...
    [dialogViewController.submitButton setImage:[[UIImageMake(@"icon_emotion") qmui_imageResizedInLimitedSize:CGSizeMake(18, 18)
                                                                                                 resizingMode:QMUIImageResizingModeScaleToFill]
                                                    qmui_imageWithTintColor:buttonTitleAttributes[NSForegroundColorAttributeName]]
...
- (void)showNormalSelectionDialogViewController {
    ...
    dialogViewController.cellForItemBlock
        = ^(QMUIDialogSelectionViewController *aDialogViewController, QMUITableViewCell *cell, NSUInteger itemIndex) {
...
- (void)showRadioSelectionDialogViewController {
    QMUIOrderedDictionary *citys = [[QMUIOrderedDictionary alloc]
        initWithKeysAndObjects:@"北京", @"吃到的第一个菜肯定是烤鸭吧！", @"广东", @"听说那里的人一日三餐都吃🐍🐸🐛🦂😋",
        @"上海", @"好像现在全世界的蟹都叫大闸蟹？", @"成都", @"你分得清冒菜和麻辣烫、龙抄手和馄饨吗？", nil];
    QMUIDialogSelectionViewController *dialogViewController = [[QMUIDialogSelectionViewController alloc] init];
    ...
    [dialogViewController addSubmitButtonWithText:@"确定" block:^(QMUIDialogViewController *aDialogViewController) {
        ...
        [aDialogViewController hideWithAnimated:YES completion:^(BOOL finished) {
            QMUIAlertController *alertController = [QMUIAlertController alertControllerWithTitle:resultString message:nil
...
- (void)showNormalTextFieldDialogViewController {
    ...
    [dialogViewController addTextFieldWithTitle:@"昵称"
                           configurationHandler:^(QMUILabel *titleLabel, QMUITextField *textField, CALayer *separatorLayer) {
                               textField.placeholder = @"不超过10个字符";
                               textField.maximumTextLength = 10;
                           }];
    [dialogViewController addTextFieldWithTitle:@"密码"
                           configurationHandler:^(QMUILabel *titleLabel, QMUITextField *textField, CALayer *separatorLayer) {
...
- (void)showReturnKeyDialogViewController {
    ...
    dialogViewController.shouldManageTextFieldsReturnEventAutomatically
        = YES;  // 让键盘的 Return 键也能触发确定按钮的事件。这个属性默认就是 YES，这里为写出来只是为了演示
    [dialogViewController addCancelButtonWithText:@"取消" block:nil];
...
- (void)showSubmitButtonEnablesDialogViewController {
    ...
    dialogViewController.enablesSubmitButtonAutomatically
        = YES;  // 自动根据输入框的内容是否为空来控制 submitButton.enabled 状态。这个属性默认就是 YES，这里为写出来只是为了演示
    [dialogViewController addCancelButtonWithText:@"取消" block:nil];
    [dialogViewController addSubmitButtonWithText:@"确定" block:^(QMUIDialogViewController *dialogViewController) {
        [QMUITips showSucceed:@"提交成功" inView:self.view hideAfterDelay:1.2];
        [dialogViewController hide];

...
- (void)initSubviews {
    ...
    NSMutableAttributedString *attributedString =
        [[NSMutableAttributedString alloc] initWithString:@"本界面以 QMUIEmotionInputManager 为例，展示 QMUIEmotionView 的功能，若需查看 "
                                                          @"QMUIEmotionView 的使用方式，请参考 QMUIEmotionInputManager。"
                                               attributes:@{
                                                   NSFontAttributeName: UIFontMake(16),
                                                   NSForegroundColorAttributeName: UIColorGray1,
                                                   NSParagraphStyleAttributeName: [NSMutableParagraphStyle qmui_paragraphStyleWithLineHeight:22]
                                               }];
...
// 布局时依赖 self.view.safeAreaInset.bottom，但由于前一个界面有 tabBar，导致 push 进来后第一次布局，self.view.safeAreaInset.bottom 依然是以存在
// tabBar 的方式来计算的，所以会有跳动，简单处理，这里通过动画来掩饰这个跳动，哈哈
- (void)viewDidAppear:(BOOL)animated {
    [super viewDidAppear:animated];
    self.toolbar.transform = CGAffineTransformMakeTranslation(0, self.view.qmui_height - self.toolbar.qmui_top);
    self.emotionInputManager.emotionView.transform
        = CGAffineTransformMakeTranslation(0, self.view.qmui_height - self.emotionInputManager.emotionView.qmui_top);
...
- (void)viewDidLayoutSubviews {
    ...
    self.descriptionLabel.frame
        = CGRectFlatMake(padding.left, self.qmui_navigationBarMaxYInViewCoordinator + padding.top, contentWidth, QMUIViewSelfSizingHeight);

    CGFloat toolbarHeight = 56;
    CGFloat emotionViewHeight = 232;
    if (self.keyboardVisible) {
        self.toolbar.frame
            = CGRectMake(0, CGRectGetHeight(self.view.bounds) - self.keyboardHeight - toolbarHeight, CGRectGetWidth(self.view.bounds), toolbarHeight);
        self.emotionInputManager.emotionView.frame
            = CGRectMake(0, CGRectGetMaxY(self.toolbar.frame), CGRectGetWidth(self.view.bounds), emotionViewHeight);
    } else {
        self.emotionInputManager.emotionView.frame
            = CGRectMake(0, CGRectGetHeight(self.view.bounds) - self.view.qmui_safeAreaInsets.bottom - emotionViewHeight,
                CGRectGetWidth(self.view.bounds), emotionViewHeight);
        self.toolbar.frame = CGRectMake(
            0, CGRectGetMinY(self.emotionInputManager.emotionView.frame) - toolbarHeight, CGRectGetWidth(self.view.bounds), toolbarHeight);
    }

    UIEdgeInsets toolbarPadding = UIEdgeInsetsConcat(UIEdgeInsetsMake(2, 8, 2, 8), self.toolbar.qmui_safeAreaInsets);
    self.textField.frame
        = CGRectMake(toolbarPadding.left, toolbarPadding.top, CGRectGetWidth(self.toolbar.bounds) - UIEdgeInsetsGetHorizontalValue(toolbarPadding),
            CGRectGetHeight(self.toolbar.bounds) - UIEdgeInsetsGetVerticalValue(toolbarPadding));
}

...
@interface QDCellSizeKeyCacheViewController () <UICollectionViewDataSource, UICollectionViewDelegateFlowLayout,
...
- (void)viewDidLayoutSubviews {
    ...
    ;
    self.collectionLayout.estimatedItemSize
        = CGSizeMake(CGRectGetWidth(self.collectionView.bounds) - UIEdgeInsetsGetHorizontalValue(self.collectionLayout.sectionInset), 300);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.attributedText = [[NSAttributedString alloc] initWithString:text attributes:@{
...
- (UICollectionViewLayoutAttributes *)preferredLayoutAttributesFittingAttributes:(UICollectionViewLayoutAttributes *)layoutAttributes {
    ...
    CGFloat resultHeight =
        [self.textLabel sizeThatFits:CGSizeMake(result.size.width - UIEdgeInsetsGetHorizontalValue(self.paddings), CGFLOAT_MAX)].height
        + UIEdgeInsetsGetVerticalValue(self.paddings);
    CGSize resultSize = CGSizeFlatted(CGSizeMake(result.size.width, resultHeight));
    NSLog(@"第 %@ 个 cell 的 preferredLayoutAttributesFittingAttributes: 被调用（说明这个 cell 的 size 重新计算了一遍），结果为 %@",
        @(self.indexPath.item), NSStringFromCGSize(resultSize));
...
- (void)layoutSubviews {
    ...
    self.textLabel.frame
        = CGRectMake(self.paddings.left, self.paddings.top, CGRectGetWidth(self.contentView.bounds) - UIEdgeInsetsGetHorizontalValue(self.paddings),
            CGRectGetHeight(self.contentView.bounds) - UIEdgeInsetsGetVerticalValue(self.paddings));
}

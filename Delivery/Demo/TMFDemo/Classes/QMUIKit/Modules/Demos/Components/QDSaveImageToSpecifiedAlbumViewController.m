...
- (void)viewDidLayoutSubviews {
    ...
    self.testImageView.frame = CGRectMake(
        CGFloatGetCenter(CGRectGetWidth(self.view.bounds), TestImageSize.width), contentMinY + 60, TestImageSize.width, TestImageSize.height);
    self.changeImageButton.frame
        = CGRectSetXY(self.changeImageButton.frame, CGFloatGetCenter(CGRectGetWidth(self.view.bounds), CGRectGetWidth(self.changeImageButton.frame)),
            CGRectGetMaxY(self.testImageView.frame) + 50);
...
- (UIImage *)randomImage {
    ...
    UIImage *resultBackgroundImage = [UIImage qmui_imageWithShape:QMUIImageShapeOval size:CGSizeMake(TestImageSize.width, TestImageSize.height)
                                                        tintColor:[QDCommonUI randomThemeColor]];
    resultImage =
        [resultBackgroundImage qmui_imageWithImageAbove:resultImage
                                                atPoint:CGPointMake(CGFloatGetCenter(resultBackgroundImage.size.width, resultImage.size.width),
                                                            CGFloatGetCenter(resultBackgroundImage.size.height, resultImage.size.height))];
    return resultImage;
}
...
- (void)saveImageToAlbum {
    if (!self.alertController) {
        self.alertController = [QMUIAlertController alertControllerWithTitle:@"保存到指定相册" message:nil
                                                              preferredStyle:QMUIAlertControllerStyleActionSheet];
        // 显示空相册，不显示智能相册
        [[QMUIAssetsManager sharedInstance]
            enumerateAllAlbumsWithAlbumContentType:QMUIAlbumContentTypeAll
                                    showEmptyAlbum:YES
                         showSmartAlbumIfSupported:NO usingBlock:^(QMUIAssetsGroup *resultAssetsGroup) {
                             if (resultAssetsGroup) {
                                 [self.albumsArray addObject:resultAssetsGroup];
                                 QMUIAlertAction *action =
                                     [QMUIAlertAction actionWithTitle:[resultAssetsGroup name] style:QMUIAlertActionStyleDefault handler:^(
                                         QMUIAlertController *aAlertController, QMUIAlertAction *action) {
                                         QMUIImageWriteToSavedPhotosAlbumWithAlbumAssetsGroup(
                                             self.testImageView.image, resultAssetsGroup, ^(QMUIAsset *asset, NSError *error) {
                                                 if (asset) {
                                                     [QMUITips showSucceed:[NSString stringWithFormat:@"已保存到相册-%@", [resultAssetsGroup name]]
                                 ...
                                 QMUIAlertAction *cancelAction = [QMUIAlertAction actionWithTitle:@"取消" style:QMUIAlertActionStyleCancel
                                                                                          handler:nil];
...
- (void)handleSaveButtonClick:(id)sender {
    if ([QMUIAssetsManager authorizationStatus] == QMUIAssetAuthorizationStatusNotDetermined) {
        [QMUIAssetsManager requestAuthorization:^(QMUIAssetAuthorizationStatus status) {
            // requestAuthorization:(void(^)(QMUIAssetAuthorizationStatus status))handler 不在主线程执行，因此涉及 UI
            // 相关的操作需要手工放置到主流程执行。

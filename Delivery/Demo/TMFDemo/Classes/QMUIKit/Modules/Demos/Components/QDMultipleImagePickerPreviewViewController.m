...
- (void)initSubviews {
    ...
    [self.originImageCheckboxButton setImage:UIImageMake(@"origin_image_checkbox_checked")
    ...
    [self.originImageCheckboxButton addTarget:self action:@selector(handleOriginImageCheckboxButtonClick:)
...
- (void)viewDidLayoutSubviews {
    ...
    self.bottomToolBarView.frame
        = CGRectMake(0, CGRectGetHeight(self.view.bounds) - bottomToolBarHeight, CGRectGetWidth(self.view.bounds), bottomToolBarHeight);
    self.sendButton.frame = CGRectSetXY(self.sendButton.frame,
        CGRectGetWidth(self.bottomToolBarView.frame) - bottomToolBarPaddingHorizontal - CGRectGetWidth(self.sendButton.frame),
        CGFloatGetCenter(bottomToolBarContentHeight, CGRectGetHeight(self.sendButton.frame)));
    _imageCountLabel.frame = CGRectMake(CGRectGetMinX(self.sendButton.frame) - 5 - ImageCountLabelSize.width,
        CGRectGetMinY(self.sendButton.frame) + CGFloatGetCenter(CGRectGetHeight(self.sendButton.frame), ImageCountLabelSize.height),
        ImageCountLabelSize.width, ImageCountLabelSize.height);
    self.originImageCheckboxButton.frame = CGRectSetXY(self.originImageCheckboxButton.frame, bottomToolBarPaddingHorizontal,
        CGFloatGetCenter(bottomToolBarContentHeight, CGRectGetHeight(self.originImageCheckboxButton.frame)));
}
    ...
    if (asset.assetType == QMUIAssetTypeAudio || asset.assetType == QMUIAssetTypeVideo) {
        ...
        if (self.originImageCheckboxButton.selected) {
            [asset assetSize:^(long long size) {
                [self.originImageCheckboxButton setTitle:[NSString stringWithFormat:@"原图(%@)", [QDUIHelper humanReadableFileSize:size]]
...
- (void)imagePreviewView:(QMUIImagePreviewView *)imagePreviewView renderZoomImageView:(QMUIZoomImageView *)zoomImageView atIndex:(NSUInteger)index {
    ...
    zoomImageView.videoToolbarMargins = UIEdgeInsetsSetBottom([QMUIZoomImageView appearance].videoToolbarMargins,
        [QMUIZoomImageView appearance].videoToolbarMargins.bottom + CGRectGetHeight(self.bottomToolBarView.frame)
            - imagePreviewView.qmui_safeAreaInsets
                  .bottom);  // videToolbarMargins 是利用 UIAppearance 赋值的，也即意味着要在 addSubview 之后才会被赋值，而在 renderZoomImageView
                             // 里，zoomImageView 可能尚未被添加到 view 层级里，所以无法通过 zoomImageView.videoToolbarMargins
                             // 获取到原来的值，因此只能通过 [QMUIZoomImageView appearance] 的方式获取
}

...
- (void)viewDidLayoutSubviews {
    ...
    self.containerView.frame
        = CGRectApplyAffineTransformWithAnchorPoint(containerRect, self.containerView.transform, self.containerView.layer.anchorPoint);

    self.toolbarView.frame
        = CGRectFlatMake(0, CGRectGetHeight(self.containerView.bounds) - kToolbarHeight, CGRectGetWidth(self.containerView.bounds), kToolbarHeight);
    self.cancelButton.frame
        = CGRectFlatMake(20, CGFloatGetCenter(CGRectGetHeight(self.toolbarView.bounds), CGRectGetHeight(self.cancelButton.bounds)),
    ...
    self.publishButton.frame = CGRectFlatMake(CGRectGetWidth(self.toolbarView.bounds) - CGRectGetWidth(self.publishButton.bounds) - 20,
        ...
        CGRectGetWidth(self.publishButton.bounds), CGRectGetHeight(self.publishButton.bounds));

    self.textView.frame
        = CGRectFlatMake(0, 0, CGRectGetWidth(self.containerView.bounds), CGRectGetHeight(self.containerView.bounds) - kToolbarHeight);
}
...
- (void)keyboardWillChangeFrameWithUserInfo:(QMUIKeyboardUserInfo *)keyboardUserInfo {
    ...
    [QMUIKeyboardManager handleKeyboardNotificationWithUserInfo:keyboardUserInfo showBlock:^(QMUIKeyboardUserInfo *keyboardUserInfo) {
        [QMUIKeyboardManager animateWithAnimated:YES keyboardUserInfo:keyboardUserInfo animations:^{
            ...
            weakSelf.containerView.layer.transform
                = CATransform3DMakeTranslation(0, -distanceFromBottom - CGRectGetHeight(self.containerView.bounds), 0);
        } completion:NULL];
...
- (void)initSubviews {
    ...
    NSMutableAttributedString *contentAttributedString = [[NSMutableAttributedString alloc]
        initWithString:
            @"QMUIKeyboardManager 以更方便的方式管理键盘事件，无需再关心 notification、键盘坐标转换、判断是否目标输入框等问题，并兼容 iPad "
            @"浮动键盘和外接键盘。\nQMUIKeyboardManager 有两种使用方式，一种是直接使用，一种是集成到 UITextField(QMUI) 及 UITextView(QMUI) 内。"
            attributes:@{
                NSFontAttributeName: UIFontMake(16),
                NSForegroundColorAttributeName: UIColorGray1,
                NSParagraphStyleAttributeName: [NSMutableParagraphStyle qmui_paragraphStyleWithLineHeight:24 lineBreakMode:NSLineBreakByCharWrapping]
            }];
...
- (void)viewDidLayoutSubviews {
    ...
    self.faceButton.frame
        = CGRectSetXY(self.faceButton.frame, CGRectGetWidth(self.toolbarView.bounds) - CGRectGetWidth(self.faceButton.bounds) - emotionRight,
            CGFloatGetCenter(CGRectGetHeight(self.toolbarView.bounds), CGRectGetHeight(self.faceButton.bounds)));
    ...
    if (CGRectGetHeight(self.view.bounds) - navigationBarHeight < contentSize.height + contentLabelInsetVertical * 2 + contentOffsetY
                                                                      + commentButtonHeight + writeReviewButtonHeight + buttonSpacing
                                                                      + buttonSectionInset * 2) {
        buttonSectionInset = (CGRectGetHeight(self.view.bounds) - navigationBarHeight - contentSize.height - contentLabelInsetVertical * 2
                                 - contentOffsetY - commentButtonHeight - writeReviewButtonHeight - buttonSpacing)
                             / 2;
    }

    self.contentLabel.frame
        = CGRectFlatMake(contentLabelInsetHorizontal, navigationBarHeight + contentLabelInsetVertical - 6, contentWidth, contentSize.height);

    self.separatorLayer.frame
        = CGRectFlatMake(0, CGRectGetMaxY(self.contentLabel.frame) + contentLabelInsetVertical, CGRectGetWidth(self.view.bounds), PixelOne);

    self.commentButton.frame
        = CGRectSetXY(self.commentButton.frame, CGFloatGetCenter(CGRectGetWidth(self.view.bounds), CGRectGetWidth(self.commentButton.bounds)),
            CGRectGetMaxY(self.separatorLayer.frame) + buttonSectionInset);

    self.writeReviewButton.frame
        = CGRectSetXY(self.writeReviewButton.frame, CGFloatGetCenter(CGRectGetWidth(self.view.bounds), CGRectGetWidth(self.writeReviewButton.bounds)),
            CGRectGetMaxY(self.commentButton.frame) + buttonSpacing);
    ...
    if (self.emotionInputManager.emotionView) {
        ...
        self.emotionInputManager.emotionView.frame = CGRectApplyAffineTransformWithAnchorPoint(
            emotionViewRect, self.emotionInputManager.emotionView.transform, self.emotionInputManager.emotionView.layer.anchorPoint);
    }
}
...
- (void)showEmotionView {
    [UIView animateWithDuration:0.25 delay:0 options:QMUIViewAnimationOptionsCurveOut animations:^{
        self.emotionInputManager.emotionView.layer.transform
            = CATransform3DMakeTranslation(0, -CGRectGetHeight(self.emotionInputManager.emotionView.bounds), 0);
    } completion:NULL];
    [self.toolbarTextField resignFirstResponder];
}
...
- (void)showToolbarViewWithKeyboardUserInfo:(QMUIKeyboardUserInfo *)keyboardUserInfo {
    if (keyboardUserInfo) {
        ...
        // 相对于表情面板
        [UIView animateWithDuration:0.25 delay:0 options:QMUIViewAnimationOptionsCurveOut animations:^{
            self.toolbarView.layer.transform
                = CATransform3DMakeTranslation(0, -CGRectGetHeight(self.emotionInputManager.emotionView.bounds) - kToolbarHeight, 0);
        } completion:NULL];
    }
}
